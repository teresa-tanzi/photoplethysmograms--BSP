...
sampling_rate = 125 ;    % c'è scritto nel readme

data = load('bidmc-ppg-and-respiration-dataset-1.0.0/bidmc_data');
...
for f = 1 : length(data.data)
	%if f == 41
	
	raw_signal = data.data(f).ppg.v;
	...
	% cerco in che valore delle y i valori vengono clippati in alto (in
	% questo dataset non è sempre lo stesso)
	
	potential_clipping_value = max(raw_signal);
	clipping_value = Inf;
	
	for i = 2 : size(raw_signal)
		if raw_signal(i) == raw_signal(i-1) && raw_signal(i) == potential_clipping_value
			clipping_value = potential_clipping_value;
		end
	end
	...
	for i = 1:size(raw_signal)
		...
		% TODO
		if raw_signal(i) >= clipping_value
			disturbed(i) = 1;
		end
	end
	...
	x = find(disturbed == 1);
	y = clipping_value * ones(length(x),1);

	scatter((x-1)/sampling_rate, y, 'm+');
	title('Raw Signal Artifact Detection');
	savefig(strcat('img/bidmc_data', int2str(f),'_raw.fig'));
	hold off;
	...
	while cont
		...
		if length(x_min) > 2	% ho individuato una nuova pwd
			...
			if length(pks) > 0 && length(vls) == length(pks)
				for j = 1 : length(pks)
